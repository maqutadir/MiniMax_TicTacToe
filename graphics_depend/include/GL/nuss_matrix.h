...
public:
	...
	static Matrix4f rotateRollPitchYaw(float angleRoll, float anglePitch, float angleYaw, int isDegree)
	{
		Matrix4f m1;
		m1 = identity();
		m1 = rotateY(angleYaw, isDegree)*rotateX(anglePitch, isDegree)*rotateZ(angleRoll, isDegree);
	...
	static Matrix4f rotateXYZ(float angleRoll, float anglePitch, float angleYaw, int isDegree)
	{
		Matrix4f m1;
		m1 = identity();
		m1 = rotateZ(angleRoll, isDegree)*rotateY(angleYaw, isDegree)*rotateX(anglePitch, isDegree);
	...
	static Matrix4f cameraMatrix1(Vector3f position, Vector3f lookAtPoint, Vector3f upVector)
	{
		Matrix4f m1;
		Vector3f u, v, n;
